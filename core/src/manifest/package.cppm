...
auto package_has_library_target(const PackageManifest& manifest) noexcept -> bool {
    return manifest.targets.iter().any([&](auto target) {
        return package_target_kind((*target)) == lito::package::PackageTargetKind::Library;
    });
}

auto package_has_host_tool_target(const PackageManifest& manifest) noexcept -> bool {
    return manifest.targets.iter().any([&](auto target) {
        return package_target_is_host_tool((*target));
    });
}
